...
class _AuditTransactionDialogState extends State<AuditTransactionDialog> {
  ...
  Future<void> _submitTransaction() async {
    ...
    try {
      ...
      // For now, let's test with hardcoded file URLs to see if the audit update works
      List<String> methodologyUrls = [];
      List<String> observationUrls = [];
      List<String> actionPlanUrls = [];
      List<String> actionEvidenceUrls = [];
      List<String> otherAttachmentUrls = [];
      
      // Test with dummy URLs if files are selected
      if (_methodologyFiles.isNotEmpty) {
        methodologyUrls.add('uploads/audit_main/test-methodology.xlsx');
        developer.log('Added test methodology URL', name: 'AuditTransactionDialog');
      }
      
      if (_observationFiles.isNotEmpty) {
        observationUrls.add('uploads/audit_main/test-observation.xlsx');
        developer.log('Added test observation URL', name: 'AuditTransactionDialog');
      }
      
      if (_actionPlanFiles.isNotEmpty) {
        actionPlanUrls.add('uploads/audit_main/test-actionplan.xlsx');
        developer.log('Added test action plan URL', name: 'AuditTransactionDialog');
      }
      
      if (_actionEvidenceFiles.isNotEmpty) {
        actionEvidenceUrls.add('uploads/audit_main/test-evidence.xlsx');
        developer.log('Added test action evidence URL', name: 'AuditTransactionDialog');
      }
      
      if (_otherAttachmentFiles.isNotEmpty) {
        otherAttachmentUrls.add('uploads/audit_main/test-other.xlsx');
        developer.log('Added test other attachment URL', name: 'AuditTransactionDialog');
      }
      
      setState(() => _uploadProgress = 'Updating audit...');
      
      // Debug: Log the collected file URLs
      developer.log('Methodology URLs: $methodologyUrls', name: 'AuditTransactionDialog');
      developer.log('Observation URLs: $observationUrls', name: 'AuditTransactionDialog');
      developer.log('Action Plan URLs: $actionPlanUrls', name: 'AuditTransactionDialog');
      developer.log('Action Evidence URLs: $actionEvidenceUrls', name: 'AuditTransactionDialog');
      developer.log('Other Attachment URLs: $otherAttachmentUrls', name: 'AuditTransactionDialog');
      developer.log('Audit Score: $auditScore', name: 'AuditTransactionDialog');
      
      // Update the audit with the new data
      final updateResponse = await _apiService.updateAuditMain(
        auditId: auditId,
        auditScore: auditScore,
        methodologyFiles: methodologyUrls.isNotEmpty ? methodologyUrls : null,
        observationFiles: observationUrls.isNotEmpty ? observationUrls : null,
        actionPlanFiles: actionPlanUrls.isNotEmpty ? actionPlanUrls : null,
        actionEvidenceFiles: actionEvidenceUrls.isNotEmpty ? actionEvidenceUrls : null,
        otherFiles: otherAttachmentUrls.isNotEmpty ? otherAttachmentUrls : null,
      );
      
      developer.log('Audit update response: $updateResponse', name: 'AuditTransactionDialog');
      ...
      developer.log('Error submitting transaction: $e', name: 'AuditTransactionDialog');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
        ...
        actions: [
          TextButton(
            ...
            child: Text(
              ...
              style: TextStyle(
                color: _isSubmitting ? AppTheme.gray500 : AppTheme.blue600,
                fontWeight: FontWeight.w600,
              ),
            ),
          ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...files.asMap().entries.map((entry) {
                    ...
                    final displaySize = fileSizeKB > 1024 ? '${fileSizeMB}MB' : '${fileSizeKB}KB';
  ...
  IconData _getFileIcon(String? extension) {
    switch (extension?.toLowerCase()) {
      ...
      default:
        return Icons.table_chart; // Default to Excel icon since we only allow Excel files
    }
  }
