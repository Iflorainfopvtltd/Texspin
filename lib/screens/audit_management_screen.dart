...
class _AuditManagementScreenState extends State<AuditManagementScreen> {
  ...
  List<Map<String, dynamic>> get _filteredAudits {
    ...
    return _audits.where((audit) => 
      audit['auditStatus']?.toLowerCase() == _selectedFilter
    ).toList();
  }
  ...
  Widget _buildFilterTabs() {
    final filters = [
      ...
      {'key': 'open', 'label': 'Open', 'count': _audits.where((a) => a['auditStatus'] == 'open').length},
      {'key': 'close', 'label': 'Closed', 'count': _audits.where((a) => a['auditStatus'] == 'close').length},
    ];
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            _selectedFilter == 'all' ? 'No Audits Found' : 'No ${_selectedFilter.toUpperCase()} Audits',
  ...
  Widget _buildAuditCard(Map<String, dynamic> audit) {
    ...
    final pendingCount = questions.where((q) => q['status'] == 'Pending').length;
    final completedCount = questions.where((q) => q['status'] == 'Approved').length;
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    LinearProgressIndicator(
                      value: questions.isNotEmpty ? completedCount / questions.length : 0,
                    ...
                    Row(
                      children: [
                        _buildProgressChip('Pending', pendingCount, AppTheme.orange100, AppTheme.orange600),
                        const SizedBox(width: 8),
                        _buildProgressChip('Completed', completedCount, AppTheme.green100, AppTheme.green600),
                      ],
                    ),
                  ],
                ),
  ...
  Widget _buildProgressChip(String label, int count, Color bgColor, Color textColor) {
    return Container(
  ...
  void _showAuditDetails(Map<String, dynamic> audit) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => AuditDetailsScreen(audit: audit),
      ),
    );
  }
  ...
  void _editAudit(Map<String, dynamic> audit) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Edit audit - Coming soon')),
    );
  }
}
        ...
        child: Column(
          ...
          children: [
            _buildDetailCard('Basic Information', [
              ...
              _buildDetailRow('Date', audit['date'] != null 
                  ? DateTime.parse(audit['date']).toString().split(' ')[0]
                  : 'N/A'),
            ...
            if (audit['auditTemplate'] != null)
              _buildDetailCard('Template Information', [
                _buildDetailRow('Template Name', audit['auditTemplate']['name']),
                _buildDetailRow('Segment', audit['auditTemplate']['auditSegment']?['name']),
                _buildDetailRow('Type', audit['auditTemplate']['auditType']?['name']),
              ]),
            const SizedBox(height: 16),
...
class _AuditQuestionsScreenState extends State<AuditQuestionsScreen> {
  ...
  List<Map<String, dynamic>> get _questions {
    return List<Map<String, dynamic>>.from(widget.audit['auditQuestions'] ?? []);
  }
  ...
  Widget _buildFilterChips() {
    final statuses = ['all', 'Pending', 'Assigned', 'Accepted', 'Rejected', 'Submitted', 'Approved', 'Revision'];
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              _selectedFilter == 'all' ? 'No Questions' : 'No $_selectedFilter Questions',
              style: const TextStyle(
                fontSize: 18,
                fontWeight: FontWeight.w500,
                color: AppTheme.gray600,
  ...
  List<Widget> _buildActionButtons(Map<String, dynamic> question, String status) {
    switch (status) {
      case 'Pending':
        return [
          _buildActionButton('Assign', Icons.person_add, AppTheme.blue600, () => _assignStaff(question)),
          _buildActionButton('Edit', Icons.edit, AppTheme.gray600, () => _editQuestion(question)),
          _buildActionButton('Delete', Icons.delete, AppTheme.red500, () => _deleteQuestion(question)),
        ];
      
      case 'Assigned':
        return [
          _buildActionButton('Reassign', Icons.swap_horiz, AppTheme.blue600, () => _reassignStaff(question)),
          _buildActionButton('Reminder', Icons.notifications, AppTheme.orange600, () => _sendReminder(question)),
          _buildActionButton('Edit', Icons.edit, AppTheme.gray600, () => _editQuestion(question)),
        ];
      ...
      case 'Submitted':
        return [
          _buildActionButton('Download', Icons.download, AppTheme.green600, () => _downloadFile(question)),
          _buildActionButton('Approve', Icons.check, AppTheme.green600, () => _approveSubmission(question)),
          _buildActionButton('Reject', Icons.close, AppTheme.red500, () => _rejectSubmission(question)),
        ];
  ...
  void _assignStaff(Map<String, dynamic> question) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Assign staff - Coming soon')),
    );
  }
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            child: const Text('Delete', style: TextStyle(color: AppTheme.red500)),
          ),
        ],
      ),
    );
