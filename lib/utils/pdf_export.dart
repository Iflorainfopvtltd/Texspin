...
class PdfExportService {
  ...
  static int _calculateWeeksPerPage() {
    ...
    final fixedColumnsWidth = phaseWidth +
        activityWidth +
        respWidth +
        approvalWidth +
        remarksWidth +
  ...
  static pw.Widget _buildGanttTable(Project project, int startWeekIndex, int endWeek, int totalWeeks) {
    ...
    for (final phase in project.phases) {
      ...
      final activitiesColumn = pw.Column(
        children: List.generate(
          totalActivitiesInPhase,
          (i) {
            final activity = phase.activities[i];
            return pw.Row(
              children: [
                _bodyCellWithBorder(activity.name, width: activityWidth, height: rowHeight),
                _bodyCellWithBorder(
                  activity.responsiblePerson.isEmpty ? 'Team' : activity.responsiblePerson,
                  width: respWidth,
                  height: rowHeight,
                ...
                pw.Row(
                  children: List.generate(displayWeekCount, (weekIndex) {
                    ...
                    final active = week >= activity.startWeek && week <= activity.endWeek;
                    return pw.Container(
                      width: weekWidth,
                      height: rowHeight,
                      decoration: pw.BoxDecoration(
                        border: pw.Border.all(color: PdfColors.black, width: 0.4),
                        color: active ? _statusColor(activity.status) : PdfColors.white,
                      ),
                    );
                  }),
                ),
